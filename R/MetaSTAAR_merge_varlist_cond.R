#' The preliminary data manipulation step for \code{MetaSTAAR_cond} given a variant list
#'
#' The \code{MetaSTAAR_merge_varlist_cond} function takes in the summary statistics file and the sparse weighted
#' of the covariance file (the output from \code{\link{MetaSTAAR_worker_sumstat}} and \code{\link{MetaSTAAR_worker_cov}})
...
#' @param variant_pos a numeric vector indicating all possible positions of the variants to be included
#' in the variant-set.
...
#' @param covcond.dir a character value indicating the master directory of the covariance files
#' for conditional analysis (the parent directory of the study-specific covariance file folders
...
#' @return \code{U_cond}: the merged conditional score statistics vector of all variants in the given variant position list
#' of interest whose combined minor allele frequency is below \code{rare_maf_cutoff},
#' adjusting for a given list of variants.
#' @return \code{cov_cond}: the merged conditional covariance matrix of all variants in the given variant position list
#' of interest whose combined minor allele frequency is below \code{rare_maf_cutoff},
#' adjusting for a given list of variants.
...
MetaSTAAR_merge_varlist_cond <- function(chr,variant_pos,study.names,sample.sizes,sumstat.dir,cov.dir,covcond.dir,
  ...
  if (max(variant_pos) <= segment * segment.size) {
    ...
    segment.num <- (floor((max(variant_pos) - 1) / segment.size) + 1) - segment + 1
    variant_pos.list <- list()
    for (num in 1:(segment.num-1)) {
      variant_pos.list[[num]] <- variant_pos[variant_pos > (segment + num - 2) * segment.size & variant_pos <= (segment + num) * segment.size]
    }
    # variant_pos.list <- variant_pos.list[lapply(variant_pos.list, length) > 0]
    ...
    results.U.list <- lapply(results.list,function(x) {x$U})
    results.U.merge <- do.call("c",results.U.list)
    results.U.nodup <- results.U.merge[!duplicated(results.info.merge)]
    results.cov.list <- lapply(results.list,function(x) {x$cov})
    results.info.nodup <- results.info.merge[!duplicated(results.info.merge),]
    if (is.null(results.info.nodup)) {
      return(NULL)
    }else if (dim(results.info.nodup)[1] == 0) {
      return(NULL)
    }
    results.info.nodup$index <- 1:dim(results.info.nodup)[1]
    results.index.list <- lapply(results.info.list, function(x) {
      if (is.null(x)) {
        integer(0)
      }else{
        left_join(x,results.info.nodup,by=c("chr"="chr",
                                            "pos"="pos",
                                            "ref"="ref",
                                            "alt"="alt",
                                            "MAC"="MAC",
    ...
    results.cov.nodup <- matrix(0, nrow = dim(results.info.nodup)[1], ncol = dim(results.info.nodup)[1])
    for (num in 1:length(results.cov.list)) {
      results.cov.nodup[results.index.list[[num]],results.index.list[[num]]] <- results.cov.list[[num]]
    }
    return(list(info=results.info.nodup[,c("chr","pos","ref","alt","MAC","MAF")],
                U_cond=results.U.nodup,
                cov_cond=as.matrix(results.cov.nodup)))
  }
  ...
  covcond.files <- paste0(covcond.dir,study.names,"/cov.cond.",trait,".",study.names,".chr",chr,".region.",region,".Rdata")
  ...
  covcond.list <- mapply(function(x,y) {
    GTPG_cond_nodup <- matrix(0, nrow = dim(variant_info_nodup)[1], ncol = dim(covcond.list[[1]]$GTPG_cond)[2])
    GTPG_cond_nodup[y,] <- x$GTPG_cond
    return(list(GTPG_cond = GTPG_cond_nodup,
                variant_info = variant_info_nodup[,c("chr","pos","ref","alt")],
                variant_adj_info = x$variant_adj_info))
  },x = covcond.list, y = variant_info_index_list, SIMPLIFY = FALSE)
  ...
  if (length(ex.index) > 0){
    if (effect.cond == "homogeneous") {
      ...
      cov.rare.merge <- Reduce("+", mapply(function(x,y) {
        (x - as.matrix(y[,11:dim(y)[2]]) %*% t(as.matrix(y[,11:dim(y)[2]])))[rv.index, rv.index][-ex.index,-ex.index]
      ...
      cov.merge <- Reduce("+", mapply(function(x,y,z) {
        cov <- (x - as.matrix(y[,11:dim(y)[2]]) %*% t(as.matrix(y[,11:dim(y)[2]])))[rv.index, rv.index][-ex.index,-ex.index]
        z.rare <- z[var.rare.index,]
        z.adj <- z[var.adj.index,]
    ...
    if (effect.cond == "homogeneous") {
      ...
      cov.merge <- Reduce("+", mapply(function(x,y,z) {
        cov <- (x - as.matrix(y[,11:dim(y)[2]]) %*% t(as.matrix(y[,11:dim(y)[2]])))[rv.index, rv.index]
        z.rare <- z[var.rare.index,]
        z.adj <- z[var.adj.index,]
        return(cov-z.rare%*%ginv(z.adj)%*%t(z.rare))
      }, x = cov.merge.list, y = sumstat.merge.list, z = GTPG_cond.list, SIMPLIFY = FALSE))
