#' The preliminary data manipulation step for \code{MetaSTAAR} given a variant list
#'
#' The \code{MetaSTAAR_merge_varlist} function takes in the summary statistics file and the sparse weighted
...
#' @return \code{U} the merged score statistics vector of all variants in the given variant position list
...
MetaSTAAR_merge_varlist <- function(chr,variant_pos,study.names,sample.sizes,sumstat.dir,cov.dir,
  ...
  if (max(variant_pos) <= segment * segment.size) {
    ...
    return(list(info=results.info.nodup[,c("chr","pos","ref","alt","MAC","MAF")],
                U=results.U.nodup,
                cov=as.matrix(results.cov.nodup)))
  }
